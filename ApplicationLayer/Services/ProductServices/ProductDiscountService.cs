	...
	public class ProductDiscountService : IProductDiscountService
	{
		...
		public async Task<Result<List<ProductDto>>> ApplyDiscountToProductsAsync(ApplyDiscountToProductsDto dto, string userId)
        {
            ...
            try
            {
                ...
                var discount = await _unitOfWork.Repository<Discount>().GetByIdAsync(dto.Discountid);
			...
			{
				await transaction.RollbackAsync();
				_logger.LogWarning(e, "ApplyDiscountToProductsAsync: Concurrency conflict for discount {DiscountId}", dto.Discountid);
				return Result<List<ProductDto>>.Fail("Products were modified by another process.", 409);
			}
            catch (Exception ex)
            {
                await transaction.RollbackAsync();
		...
		public async Task<Result<List<ProductDto>>> RemoveDiscountFromProductsAsync(List<int> productIds, string userId)
		{
			...
			{
				await transaction.RollbackAsync();
				_logger.LogWarning(e, "RemoveDiscountFromProductsAsync: Concurrency conflict for products: {Products}", string.Join(", ", productIds));
				return Result<List<ProductDto>>.Fail("Products were modified by another process.", 409);
			}
			catch (Exception ex)
			{
		...
		public async Task<Result<bool>> AddDiscountToProductAsync(int productId, int discountId, string userId)
		{
			...
			{
				await transaction.RollbackAsync();
				_logger.LogWarning(e, "AddDiscountToProductAsync: Concurrency conflict for product {ProductId}", productId);
		...
		public async Task<Result<bool>> UpdateProductDiscountAsync(int productId, int discountId, string userId)
		{
			...
			{
				_logger.LogWarning(e, "UpdateProductDiscountAsync: Concurrency conflict for product {ProductId}", productId);
				return Result<bool>.Fail("Product was modified by another process.", 409);
			}
			catch (Exception ex)
			{
		...
		public async Task<Result<bool>> RemoveDiscountFromProductAsync(int productId, string userId)
		{
			...
			{
				_logger.LogWarning(e, "RemoveDiscountFromProductAsync: Concurrency conflict for product {ProductId}", productId);
				return Result<bool>.Fail("Product was modified by another process.", 409);
			}
			catch (Exception ex)
			{
