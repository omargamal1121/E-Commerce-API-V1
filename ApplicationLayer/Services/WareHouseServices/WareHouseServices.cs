...
namespace Application.Services.WareHouseServices
{
	public class WareHouseServices : IWareHouseServices
	{
		...
		public WareHouseServices(ICacheManager cacheManager, IAdminOpreationServices adminOpreationServices, ILogger<WareHouseServices> logger, IUnitOfWork unitOfWork)
		{
		...
		public async Task<Result<WareHouseDto>> CreateWareHouseAsync(string userid, WareHouseDto wareHouse)
		{
			...
			var createdwarehouse = new WareHouseDto { Id = iscreated.Id, Name = iscreated.Name, Address = iscreated.Address, Phone = iscreated.Phone };
		...
		public async Task<Result<List<WareHouseDto>>> GetAllWareHousesAsync()
		{
			...
			var warehousesdto = await warehousereult.Where(w => w.DeletedAt == null).Select(i=>new WareHouseDto { }).ToListAsync();
		...
		public async Task<Result<WareHouseDto>> GetWareHouseByIdAsync(int id)
		{
			...
			var warehousedto = new WareHouseDto { Id = warehouse.Id, Name = warehouse.Name, Address = warehouse.Address, Phone = warehouse.Phone }	;
		...
		public async Task<Result<WareHouseDto>> ReturnRemovedWareHouseAsync(int id, string userid)
		{
			...
			try
			{
				...
				{
					Id = deletdwarehouse.Id,
					Name = deletdwarehouse.Name,
					Address = deletdwarehouse.Address,
					Phone = deletdwarehouse.Phone
				};
