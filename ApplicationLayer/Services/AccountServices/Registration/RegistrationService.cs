...
namespace Application.Services.AccountServices.Registration
{
    public class RegistrationService : IRegistrationService
    {
        ...
        public async Task<Result<RegisterResponse>> RegisterAsync(RegisterDto model)
        {
            ...
            try
            {
                // Check for conflicts with existing email or username
                ...
                if (existingByName != null && existingByName.DeletedAt == null)
                {
                    return Result<RegisterResponse>.Fail("This username already exists.", 409);
                }

                // If we found deleted accounts, anonymize them to free up the credentials
                if (existingByEmail != null || existingByName != null)
                {
                    var usersToRename = new List<Customer> { existingByEmail, existingByName }
                        .Where(u => u != null && u.DeletedAt != null)
                        .DistinctBy(u => u.Id);

                    foreach (var u in usersToRename)
                    {
                        var suffix = Guid.NewGuid().ToString().Substring(0, 8);
                        u.Email = $"deleted_{suffix}_{u.Email}";
                        u.UserName = $"deleted_{suffix}_{u.UserName}";
                        u.NormalizedEmail = _userManager.NormalizeEmail(u.Email);
                        u.NormalizedUserName = _userManager.NormalizeName(u.UserName);

                        var renameResult = await _userManager.UpdateAsync(u);
                        if (!renameResult.Succeeded)
                        {
                            return Result<RegisterResponse>.Fail("Registration failed due to historical record conflict.", 500);
                        }
                    }
                }
				...
				if (request != null)
				{
			    	...
			    	BackgroundJob.Enqueue<IAccountEmailService>(e => e.SendValidationEmailAsync(customer.Email,customer.Id,encodedToken,baseUrl));
				}
