...
namespace ApplicationLayer.Services.PaymentProvidersServices
{
	...
	public class PaymentProvidersServices : IPaymentProvidersServices
	{
		...
		public async Task<Result<bool>> UpdateAsync(int id, UpdatePaymentProviderDto dto, string userId)
		{
			...
			try
			{
				...
				if (!string.IsNullOrEmpty(dto.Name) &&paymentprovider.Name.ToLower() != dto.Name.ToLower())
				{
				...
				if (!string.IsNullOrEmpty(dto.ApiEndpoint) &&!string.Equals(paymentprovider.ApiEndpoint, dto.ApiEndpoint))
				{
				...
				if (!string.IsNullOrEmpty(dto.PublicKey) && !string.Equals(paymentprovider.PublicKey, dto.PublicKey))
				{
					updatedFields.Add($"PublicKey changed");
					paymentprovider.PublicKey = dto.PublicKey;
				}

				if (!string.IsNullOrEmpty(dto.PrivateKey) && !string.Equals(paymentprovider.PrivateKey, dto.PrivateKey))
				{
					updatedFields.Add($"PrivateKey changed");
					paymentprovider.PrivateKey = dto.PrivateKey;
				}

				if (!string.IsNullOrEmpty(dto.Hmac) && !string.Equals(paymentprovider.Hmac, dto.Hmac))
				{
					updatedFields.Add($"Hmac changed");
					paymentprovider.Hmac = dto.Hmac;
				}

				if (!string.IsNullOrEmpty(dto.IframeId) && !string.Equals(paymentprovider.IframeId, dto.IframeId))
				{
